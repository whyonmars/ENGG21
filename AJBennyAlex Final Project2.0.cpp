...
double getRegression(vector <double> a, vector <double> b, vector <double> datavec)
{
  ...
  for (int i = 0; i < n/2; i++)
  {
    cout << "  " << a[i]; //x-value
    cout << "\t\t    " << b[i]; //y-value
    cout << "\t\t    "<<value2*a[i]+value1; //Residuals
    cout << "\t    "<<(b[i]-(value2*a[i]+value1))*(b[i]-(value2*a[i]+value1)); //SSE
    cout << "\t    "<<(b[i]-(sum_Y/(n/2)))*(b[i]-(sum_Y/(n/2)))<< endl; //SST
  }
    for (int z = 0; z < n/2; z++)
  {
    sumSSE += (b[z]-(value2*a[z]+value1))*(b[z]-(value2*a[z]+value1));
    sumSST += (b[z]-(sum_Y/(n/2)))*(b[z]-(sum_Y/(n/2)));
    sumResiduals += (value2*a[z]+value1);
  }
    meanResiduals = (sumResiduals)/(n/2);
    cout << "\nSummation SSE: " << sumSSE;
    cout << "\nSummation SST: " << sumSST;
    cout << "\nSummation of Residuals: " << sumResiduals;
    cout << "\nMean of Residuals: " << meanResiduals;
